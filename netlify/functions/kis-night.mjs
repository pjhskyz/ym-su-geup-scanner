// netlify/functions/kis-night.mjs  (v2 — 월물 자동 탐지)
...
//       /.netlify/functions/kis-night?probe=1      → 모든 후보 코드 응답 현황 (진단용)
...
export default async (req) => {
  ...
  try {
    ...
    if (!code) {
      ...
      if (!code) return json({ error: "유효한 월물 코드를 찾지 못했습니다 — ?probe=1 로 진단하세요" }, 502);
